...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        // 使用 Resend 的测试域名，立即可用！
        from: 'Property Inspection <onboarding@resend.dev>',
        to: [to],
        subject: subject,
        html: emailHtml,
      }),
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        .booking-details { background-color: #f8fafc; border-radius: 8px; padding: 20px; margin: 20px 0; border-left: 4px solid ${buttonColor}; }
        .detail-row { margin: 10px 0; display: flex; justify-content: space-between; }
        .detail-label { font-weight: bold; color: #374151; }
        .detail-value { color: #6b7280; }
        .highlight { color: ${buttonColor}; font-weight: bold; }
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          <h2 style="color: ${buttonColor};">${title}</h2>
          
          <p>Dear <span class="highlight">${booking.customerName}</span>,</p>
          <p>${message}</p>
          
          <div class="booking-details">
            <h3 style="margin-top: 0; color: #374151;">Booking Details | 预约详情</h3>
            <div class="detail-row">
              <span class="detail-label">Order Number | 订单号:</span>
              <span class="detail-value highlight">${booking.orderNumber}</span>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
              ...
              <span class="detail-value highlight">${booking.preferredDate} ${booking.preferredTimeSlot}</span>
            </div>
            <div class="detail-row">
          ...
          <div class="chinese">
            <p>亲爱的 <span class="highlight">${booking.customerName}</span>，</p>
            <p>${message.split(' | ')[1]}</p>
            <p>如有任何疑问，请随时联系我们。</p>
          </div>
        </div>
        ...
        <div class="footer">
          <p><strong>Best regards | 此致敬礼</strong><br>
          Property Inspection Team | 房产检查团队</p>
          <p style="margin-top: 15px; font-size: 12px;">This is an automated message | 这是一封自动发送的邮件</p>
        </div>
      </div>
    </body>
    </html>
